...
try:
    ...
    ExecuteByNameUntilAdapter = TypeAdapter(ExecuteByNameUntil)
    MODELS_AVAILABLE = True
except ImportError:
    MODELS_AVAILABLE = False

execute_by_name_until_model_schema = json.loads(
    r"""{
  "type" : "string",
  "oneOf" : [ {
...
execute_by_name_until_model_schema.update({"definitions": MODEL_DEFINITIONS})

execute_by_name_until_faker = JSF(
    execute_by_name_until_model_schema, allow_none_optionals=1
)


class ExecuteByNameUntilStub:
    """ExecuteByNameUntil unit test stubs."""

    @classmethod
    def create_json(cls):
        """Create a dict stub instance."""
        return execute_by_name_until_faker.generate(
            use_defaults=True, use_examples=True
        )

    @classmethod
    def create_instance(cls) -> "ExecuteByNameUntil":
        """Create ExecuteByNameUntil stub instance."""
        ...
        if json is None:
            ...
            backup_faker = JSF(
                ExecuteByNameUntilAdapter.json_schema(), allow_none_optionals=1
            )
            json = backup_faker.generate(use_defaults=True, use_examples=True)
        return ExecuteByNameUntilAdapter.validate_python(
